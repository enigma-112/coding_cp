...
#pragma GCC target("sse4")#include "bits/stdc++.h"
...
#define ptvs(v) for (auto & item: v) cout << item << " ";
cout << "\n";
#define pt(x) cout << ((x)) << "\n";
#define pt2(x, y) cout << (x) << " " << (y) << "\n";
#define cinvll(v, start, end) vll v(end + 1);
...
template < typename T > T power(T a, T b, T modulo) {
  if (b == 0) return 1;
  if (b == 1) return a;
  else {
...
ll mod_inv(ll a) {
  return (power < ll > (a, mod - 2, mod)) % mod;
}
ll ncr(ll n, ll r, vll & fact) {
  if (r > n or n < 0 or r < 0) return 0 LL;
  return (((fact[n] % mod) * (mod_inv(fact[n - r]) % mod)) % mod * (mod_inv(fact[r]) % mod)) % /*Author:https://www.youtube.com/channel/UCAVYawHY1vnZXjtIE4Qv3mw*/ mod;
}
ll n;
vector < vi > tree;
vi cc;
...
void DFS(ll nd) {
  for (auto & to: tree[nd]) {
    ...
    DFS(c);
    for (auto & it: store[c]) {
      store[nd][cc[nd] * 1 ll * it.first] += it.second;
    }
  }
}
int main() {
  ...
  w(q) {
    ...
    cin >> nd >> w;
    nd = shrink[nd];
    auto itr = already[nd].lower_bound(pll {
      w,
      -1
    });
    ...
    for (auto & i: store[nd]) {
      if (i.first and w % i.first == 0) {
        p += (w / i.first) * i.second * 1 ll;
      }
    }
    already[nd].insert(pll {
      w,
      w - p
    });
    pt(w - p);
